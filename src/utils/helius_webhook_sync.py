...
async def sync_helius_webhook(
    ...
    """
    Sync Helius webhook with current whale wallets.
    
    1. GET current webhook config
    2. Compare with smart_money_wallets.json
    3. If mismatch - PUT update
    ...
    async with aiohttp.ClientSession() as session:
        ...
        try:
            ...
            if not webhooks:
                logger.warning("[HELIUS_SYNC] No webhooks found! Need to create one.")
                return False
        ...
        # 3. Addresses mismatch - need to update!
        missing = expected_addresses - current_addresses
        extra = current_addresses - expected_addresses
        
        logger.warning(f"[HELIUS_SYNC] ADDRESSES MISMATCH!")
        logger.warning(f"[HELIUS_SYNC]   Missing: {len(missing)} addresses")
        logger.warning(f"[HELIUS_SYNC]   Extra:   {len(extra)} addresses")
        logger.warning(f"[HELIUS_SYNC]   Expected: {len(expected_addresses)}, Got: {len(current_addresses)}")
...
async def verify_webhook_addresses(
    helius_api_key: str | None = None,
    expected_count: int = 99,
) -> tuple[bool, int]:
    """
    Quick check - does webhook have expected number of addresses?
    
    Returns: (is_ok, actual_count)
    """
    api_key = helius_api_key or os.getenv("HELIUS_API_KEY")
    if not api_key:
        return False, 0
