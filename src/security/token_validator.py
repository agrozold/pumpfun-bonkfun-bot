...
from ..data_providers.helius_holder_client import HeliusHolderClient, HolderAnalysis
...
class TokenValidator:
    def __init__(
        ...
        self._helius = HeliusHolderClient() if enable_holder_check else None
    ...
    async def validate(self, mint: str) -> TokenValidation:
        ...
        if self._helius and self._helius.api_key:
            tasks.append(("holders", self._helius.get_token_holders(mint)))
        ...
        if holder_result:
            if holder_result.top_10_concentration > self.max_holder_concentration:
                rejection_reasons.append(f"Helius top10: {holder_result.top_10_concentration:.1f}%")
            if holder_result.top_holder_pct > 30:
                rejection_reasons.append(f"Top holder: {holder_result.top_holder_pct:.1f}%")
