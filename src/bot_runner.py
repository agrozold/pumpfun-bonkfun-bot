...
async def start_bot(config_path: str):
    ...
    try:
        trader = UniversalTrader(
            ...
            sell_priority_fee=cfg.get("priority_fees", {}).get("sell_priority_fee", 2_000_000),
            sell_jupiter_priority_fee=cfg.get("priority_fees", {}).get("sell_jupiter_priority_fee", 200_000),
            ...
            take_profit_percentage=cfg["trade"]["take_profit_percentage"],
            stop_loss_percentage=cfg["trade"]["stop_loss_percentage"],
            max_hold_time=cfg["trade"].get("max_hold_time"),
            moon_bag_percentage=cfg["trade"]["moon_bag_percentage"],
            price_check_interval=cfg["trade"]["price_check_interval"],
            # Trailing Stop-Loss (TSL) configuration
            tsl_enabled=cfg["trade"]["tsl_enabled"],
            tsl_activation_pct=cfg["trade"]["tsl_activation_pct"],
            tsl_trail_pct=cfg["trade"]["tsl_trail_pct"],
            tsl_sell_pct=cfg["trade"]["tsl_sell_pct"],
        tp_sell_pct=cfg["trade"]["tp_sell_pct"],
            dca_enabled=cfg["trade"]["dca_enabled"],
