"""
Test script for the new BagsLogsListener.

This listener subscribes directly to Meteora DBC program logs
and fetches full transactions to parse token creation data.

PumpPortal does NOT send bags.fm tokens, so this is the correct approach!
...
async def test_bags_logs_listener():
    """Test the specialized BagsLogsListener."""
    print("=" * 80)
    print("BagsLogsListener Test - Direct Meteora DBC Subscription")
    print("=" * 80)
    print()
    print(f"Program ID: {BagsAddresses.PROGRAM}")
    print()
    print("This listener:")
    print("  1. Subscribes to logsSubscribe for Meteora DBC program")
    ...
    print("Creating BagsLogsListener...")
    try:
    ...
    print()
    print("Starting to listen for bags.fm tokens...")
    ...
    async def on_new_token(token_info: TokenInfo) -> None:
        ...
        print()
        print("=" * 80)
        print(f"🎒 BAGS TOKEN #{token_count}")
        print("=" * 80)
    ...
    try:
        ...
        print()
        print(f"Stopped. Detected {token_count} bags.fm tokens.")
    except Exception as e:
        print(f"Error: {e}")
        import traceback
        traceback.print_exc()
...
if __name__ == "__main__":
    print()
    asyncio.run(test_bags_logs_listener())
